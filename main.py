...
def extract_features(image):
    ...
    # Convert grayscale image to RGB by repeating the grayscale channel across all 3 channels
    image_rgb = cv2.cvtColor(image_resized, cv2.COLOR_GRAY2BGR)
    
    # Convert the image to a numpy array and normalize pixel values to [0, 1]
    feature = np.array(image_rgb, dtype=np.float32) / 255.0
...
while True:
    ...
    try:
        ...
        for (p, q, r, s) in faces:
            ...
            cv2.putText(im, f'{prediction_label}', (p - 10, q - 10),
